...
import { Categories, IFilteredProducts, IProduct, Product } from "@/types";
...
export const getProductsData = async (): Promise<IProduct | undefined> => {
  try {
...
export const getProductsByQuery = async (
  ...
  try {
    const res = await fetch(
      `${NEXT_PUBLIC_BASE_URL}/products?category=${filters.category}&=${filters.sort}&=${filters.search}&=${filters.minPrice}&=${filters.maxPrice}`,
      {
        next: { revalidate: 10 },
      }
    );
